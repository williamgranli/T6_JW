...
%big problem in industry implementing change
%could a reason for this be because they're trying to fit one thing onto something different. Focus on implementation rather than what the changes are
...
%getting too close, potential bias
%change manager bias, if they implemented
%generalisability 

%Check text for more.
%% Talk about generalisability tomorrow with willski

%gatekeeper to help sample, he could, in some way, select persons that are positive. We give up control to the judgement of the gatekeeper

%only doing it at EPG.
%doing it at different times, only one company -> generalise
