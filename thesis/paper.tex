...
%%Describe the change 
The module axhttp serves to provide a Common Gateway Interface which allows the camera to dynically load data to web pages. The previous version of axhttp was re-implemented in version 2.0, to correspond with the new design standards. Additional functionality was also added to the module.
...
%%Describe the change 
The API interfaces experienced the most change between the two versions. (<-what do you mean by this?) Three of the older interfaces were deprecated in favour of the API architecture (<-what do you mean?) and the rest of the API is now comprised of new interfaces. 

The exception to this appears to be the image capture interface which only experienced a deprecation of a single feature. Each new feature is either accessed directly or via an interface which acts as a facade. 
