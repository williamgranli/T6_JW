...
The data were collected primarily through inspections of the source code and the archival analysis of the accompanying documentation. The source code was comprised of API versions. Each version of the API also had its own respective documentation. The documentation was inspected and organised to find explanations of the features offered by the API. ( "Not sure if this sentence fits here") 
...
The API code was inspected sequentially, starting with version 1.4. Both versions of the API underwent the same inspection. This involved extracting methods names, enums, data structures which were then subsequently stored in spreadsheets. Crosscase ( "Cross") analysis was then performed on both sets of data retrieved from different versions of the API \cite{seaman1999qualitative}. Changes were grouped into categories that best represented what the change achieved and were added to the spreadsheet. ( "add something here, following is not a full sentence") For example, additions, modifications and removals. After a change was found in the code, we identified the related documentation changes, or explanations that were relevant to that change and added them alongside their corresponding changes in the spreadsheets.

%William has reviewed this far!!!!!!!!!!
...
%%%%****** This stuff is not part of the section, but we might want to use it somewhere, if we don't, remove it! ********
...
%Where does this go -- Do we want to put this in anywhere?
