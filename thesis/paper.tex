...
In order to answer RQ1, we categorised the changes based on the 12 maintenance categories \cite{chapin2001types}. Initially we created predictions of the changes from inspecting the source code and accompanying documentation. Following the interview we compared our results with response from the interview to decide the actual maintenance categories of the changes. \ref{table:Categories} shows the predicted and actual maintenance categories.

The changes have been divided up into four specific categories, namely: new, modified, removed and general trends. New and modified concern additions or modifications to modules, removed are features which have been removed entirely without deprecating and general trends are broad changes which do not directly affect functionality. 
