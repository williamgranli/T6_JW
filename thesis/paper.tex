...
% Given that there is a combination of different interacting elements that do not only concern interfaces to implementations, we would argue that an API can be described as a system \cite{761853,5733835}.
...
% 1. Refer back to background (e-type)
% 2. Make the case for why our API is an e-program

%Specifics about program type etc here
Based on this, we consider APIs to be E-type systems, since they are heavily dependant on the environment and its changes. 
It is therefore not possible to create an exact specification of any non-trivial API. 
