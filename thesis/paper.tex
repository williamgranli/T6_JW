...
%---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
% %Background and need for study, make it interesting
% Evolving a software system is a precipitous activity. Many software evolution theories have been created to assist changing systems. However, as modern systems are making greater use of APIs, do these software evolution theories apply to them?
% %Other studies
% Other studies have investigated why changes are made to APIs and how these changes affect systems, however, they do not attempt to understand the driving forces behind the evolution of an API according to software evolution theories. Understanding the driving forces of API evolution can potentially improve API design and aid communication between all aspects of a system's feedback loop.
% % How do we do it?
% Our study utilises a case-study to identify the types of changes that have driven the evolution of an existing API, belonging to a case company. 
% % Our results and future suggestions
% The results of this analysis showed that the largest driving force of API evolution is the desire for new functionality. It highlighted the fact that not all software evolution theories are applicable to APIs. We found that changes happen sporadically rather than slowly over time and the contradiction of a software evolution law that states the opposite is true. The research further supported the importance of a feedback loop of a software system, which is something that company A also wants. The study concludes by proposing suggestions for future studies based on our findings.
%---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

%What I did: 
       %Tried to make the motivation/purpose more clear/'''''''crisp''''''''''
       %Removed the background/other studies stuff. Don't think that's needed in abstract.
       %Made it a bit more bold/selling
       %Re-worded the results a bit
       %Removed the last sentence, don't think we have to mention that we propose future work (since everyone does)
...
%Motivation/Purpose
       ...
       %Why should someone read your entire study?
Evolving an API is a precipitous activity, mainly because of the great implications modifications to it can have on the users of it. As modern systems are making greater use of APIs, software development organisations must take an empirical and scientific approach to the way they manage the evolution of their APIs. 
%Problem Statement
       %What is your research trying to better understand or solve?
       %What is the scope of your study - a general problem, or something specific?
       %What is your main claim or argument?
If no attempt at analysing or quantifying the evolution of an API is undertaken, there will be a lacking understanding of the evolution, and possible improvements to the maintenance strategy will be difficult to identify. We believe, that long-standing software evolution theories can provide additional insight to the field of APIs, and be of great use to companies maintaining APIs. 
%We believe, that by analysing APIs using existing software evolution theories, one can solve this problem. 
%Method
In this case study, we conduct a qualitative investigation on what drives the evolution of an industry company's existing API, by examining two versions of the API code. The changes were analysed based on two software evolution theories, and the extent to which we could reverse engineer the change decisions were tested by interviewing the designer of the API. 
%Results
The results of this analysis show that the largest driving force of API evolution is the desire for new functionality. Our findings which show that changes happen sporadically, rather than continuously,invalidate a law of software evolution, in the context of APIs. We also found that change decisions were highly reverse engineerable, and identified the feedback loop of an API to be the most major area of improvement. 
%---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------


%Stuff that should be improved:
       %double 'it' in first sentence
       %Greater use = more ppl use?
       %does undertaken fit with attempt
       %It's 239 words now... should try to cut it to 200. (maybe 2nd sentence of method can be removed) (maybe results can be shortened a bit)
...
Additions or re-implementations of functionality are defined to be enhancive changes. In addition to this, we have included deprecations that have been re-implemented to also be enhancive changes.
...
%Conclusion
Given that over half of the changes were identified to be enhancive, we conclude that enhancive types of change are the primary drivers of API evolution. 
...
Adaptive changes are defined to be changes involving the technologies or resources used by a system. 
%Data and examples
A single change was identified as being adaptive; the inclusion of new 3\textsuperscript{rd} party libraries. Libraries were added for audio and data communication. The pre-existing low-level library was further incorporated into the API.
%Conclusion 
...
\textbf{Groomative  }
%Define
Groomative changes are defined to be changes that aim to immediately improve maintainability.
%Data and examples
