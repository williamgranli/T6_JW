...
The API code was inspected sequentially, starting with version 1.4. Both versions of the API underwent the same inspection. This involved extracting methods signatures, enums, structs, typedefs and macros, which were then subsequently stored in spreadsheets. The extraction involved a lot of manual work, but Git's \cite{git} diff command was used to identify changes in files which exist in both versions. After a change was identified in the code, a description of the corresponding change in the documentation was added to the spreadsheet. The ID, module, file and description was logged for each change stored in the spreadsheet. The data collection from code and documentation was performed independently by both researchers, to reduce the risk of human errors affecting the results. If the data collected by each researcher differed, the cause of the discrepancy was investigated and resolved. 
...
The interview was conducted using a semi-structured approach \cite{robson2002real}. Structure to the interview was provided by organising it around the theories \cite{chapin2001types} \cite{lehman1980programs}. Investigative and open-ended questions related to each category and law were asked to provide a basis for comparison between the answers and our initial analysis. The interviewee was encouraged to speak freely, even if it required a change in direction or topic. This was allowed to fulfil our exploratory 
goal of the study. 
...
%Codes
Each change identified in the spreadsheet was coded based on what module the change occurred in and two other characteristics: a) if the change added, removed or modified functionality, or b) if the change was a cosmetic change.
%Concepts
...
%Theory
These categories were then used to identify the most interesting trends in the evolution of the API. These trends were later used as a discussion point during the interview.

%RQ1
%We have all the changes and our suggested theories. Categorise them now
The categories identified previously were then coded according to the software maintenance categories \cite{chapin2001types}. The process used followed the decision tree for types \cite{chapin2001types}. Each of the previously identified categories was given a main category of maintenance and a subcategory if appropriate. This was then used to answer \textbf{RQ1}.

Our ability to correctly predict the motives compared to the actual motives of the designers which were made clear during the interviews, will serve as the basis for answering \textbf{RQ1.1}.

%Description of interview analysis
The results of the interview were analysed and categorised in the same way that our identified changes had been. Validation of our findings, by comparing our categorisation against the interviewee's categorisation, was performed at this point. The results of which are used to answer \textbf{RQ1}, \textbf{RQ1.1} and \textbf{RQ2}.

%Lehman's - RQ2
The analysis of both the trends and the changes, combined with information gathered during the interview allowed us to answer \textbf{RQ2} by determining if Lehman's laws are applicable to APIs \cite{lehman1980programs}.

%RQ1.1
The final stage of analysis was conducted once all the data had been analysed. To answer \textbf{RQ1.1}, we compared our analysis and interpretation of the change intents against the opinions expressed during the interview. Success was determined based on the amount of correct interpretations made about the intent behind the changes.
