...
%Results
The results of this analysis show that the largest driving force of the APIs evolution was the desire for new functionality. Our findings which show that changes happen sporadically, rather than continuously, appear to contradict the law of Conservation of Organisational Stability in the context of APIs. We also found that it is possible to reverse engineer change decisions and in doing so, identified that the feedback loop of an API is an important area of improvement. 
\end{abstract}
...
\smallskip \noindent
\textbf{External validity  } Since only two versions of the API were analysed, we cannot claim any generalisability for our study. We do, however, believe that our study can offer comparability for companies that chose to do a similar analysis on their API. 
...
%Concusion
Preventive types of change appear to drive the evolution this API more strongly than other types of change. This could be due to the fact that as the API grows, the need for higher maintenance also increase, a fact that also seems to be supported by Lehman's laws.
...
%Conclusion
We therefore conclude that reformative changes, while important for the usability of the API, they did not drive the evolution of the API.
