...
The data were collected primarily through inspections of the source code and the archival analysis of the accompanying documentation. The source code was comprised of two versions of an API which the company had developed. An older version, 1.4, which is currently released and a newer version, 2.0, which has recently been deployed. Each version of the API had its own respective documentation. The documentation was inspected and organised to find explanations of the features offered by the API. 
...
More specifically, the approach undertaken was divided into three steps where the changes were first categorised based by following the decision tree for types \cite{chapin2001types}. The changes were then mapped according to the laws presented by \cite{lehman1980programs}. The changes were then analysed from a high-level point of view, the main goal of which was to identify trends in the changes. The final step involved analysis of both the trends and the changes. During this stage the hypotheses were generated and formulated. The same approach was taken to reduce the risks of research bias and human error in the data collection was also performed during results analysis. 
...
\section{Discussion}  have been provided valuable insight into the motives behind the evolution of an API. The results show that APIs definitely can be studied from the point of view of long-established software evolution theories. Based on the results, we have formulated the subsequent outcomes. 
...
All changes whose main motive was to either add or remove functionality were accurately predicted. These changes were the most major ones, and it was expressed during the interview that these types of changes main driving factor behind the development of the new version. In summary, eleven out of a total thirteen changes were correctly predicted. It is also interesting to note that all changes which were not assigned any sub-category were correctly predicted. This supports the fact that such changes have a clearer intent and are perhaps more well-defined. 

\smallskip \noindent
\textbf{``Structural'' reverse engineerable to a lesser extent} 
Three out of five changes which did not add functionality, but rather were introduced to reduce complexity and increase quality were correctly predicted. The two changes which were incorrectly predicted were wrong because the main motivation was confused with the secondary intent. This shows that, although the prediction was incorrect, the general reasoning was correct. Our predictions were accurate enough to allow us to form a reliable conclusion which provided valuable discussion with the case company. 
