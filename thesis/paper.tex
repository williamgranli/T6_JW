...
%%What she said 
The interviewee expressed to us that the main driving force behind this change was to re-implement and add new functionality to the axhttp module. Re-designing the module to take advantage of the new design was an added bonus.
...
%%What she said
The interviewee did shed some light on why this change occurred. We were informed that in order to continue using this feature in 2.0 and in newer versions of the camera hardware, it would have to be re-implemented from scratch. An impact assessment was undertaken to determine if the removal of the function would cost significant time and money. Ultimately it was deemed appropriate and the function was removed from the API.
...
%%Describe the change 
As with many APIs, Axis included a selection of example programs to assist developers with creating applications. This trend continued in version 2.0, but there were some significant improvements. The number of example programs increased, as did the amount of comments accompanying the code which has lead to an overall more robust set of examples.
