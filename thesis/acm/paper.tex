...
% 1st. author
\alignauthor
John Burchell\titlenote{Main author}\\
...
% 2nd. author
\alignauthor
William Granli\titlenote{Main author}\\
       \affaddr{Computer Science and Engineering}\\
       \affaddr{University of Gothenburg}\\
       \affaddr{Gothenburg, Sweden}\\
...
\category{D.2.7}{Software Engineering}{Distribution, Maintenance, Enhancement}[Restructuring, reverse engineering, and reengineering]
...
%%Does this paragraph below belong here or somewhere else? Feels a bit odd!
...
RQ1 - How do companies actively drive API evolution?
%%Intro to this particular quesiton

Overwhelmingly, the primary driving factor behind API evolution appears to have been the desire for new API functionality. We found strong evidence to suggest that most, if not all of the additions, re-implementations and functional changes were undertaken to include functionality that the partners desired. Specifically we found that 53.8\% of the changes were enhancive changes, 7.7\% were adaptive and the remaining 38.5\% were changes specifically aimed at improving the maintainability of the API.

These data seem to align with the comments made during the interview. For example, the interviewee expressed a desire to have features be added to the API more rapidly and more quickly than they have previously. This sentiment was also expressed to the interviewee by certain partners who suggested that the API was too stable and citing their wish for more features and more frequent updates to the API.

This leads to the conclusion that the inclusion of new features and the maintainability of an API are two very strong drivers of API evolution.


%%% Notes and thoughts
Consistent with Lehmans Laws - Continuing growth, feedback systems seem to be very important.
Desire from some partners to have more have even more additional features etc

Business rules and motives are not taken into consideration apparently when creating these APIs. They probably should be!

%See if our numbers match up with previous studies who said roughly 70\% of changes to an API were refactorings. That number is a little high in our case, but it's close!


RQ1.1 - To what extent can we reverse engineer API change decisions?
Roughly 78\% of our categorisation matched the actual categorisation of the changes. While it is true that we did not manage to successfully categorise every single change, we did categorise a high enough percentage to form a solid conclusion; it is possible to reverse engineer the majority of change decisions using this categorisation technique.

For future studies, this technique could be improved by performing more actions along side the code review. For example, having access to commit logs, design meetings and other such documentation would allow a much more thorough determination of intents to be performed. 

RQ2
What this means for industry:
	Greater emphasis should be placed upon communicating with the end users / application developers. APIs should be made with maintainability and extendibility in mind as they seem to be what changes the most between versions.
...
Interesting that the majority of the changes seem to be driven either by management (Suggested that it comes from partners) or the partners directly. There was also internal pressure to improve the API which came about from Axis developers trying to create their own applications.
